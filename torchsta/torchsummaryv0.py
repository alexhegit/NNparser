...
def summary(
    ...
    *args: Any,
    ...
    col_names: Sequence[str] = ("input_size","output_size","kernel_size", "stride_size", "pad_size", "num_params"),
    col_width: int = 25,
    depth: int = 3,
    ...
    **kwargs: Any
    ...
    # ======================multiple input    
        ...
        for item in input_data:
            if isinstance(input_data, torch.Tensor):# single input
                input_size = get_correct_input_sizes(input_data.size())
                x = [input_data.to(device)]
            elif isinstance(input_data, tuple):#add all inputs in tensor
            ...
            elif isinstance(input_data, list): # add args
                x.append(input_data)
    ######################### 0620 ###########    
    elif isinstance(input_data, (list, tuple)):
    ...
    for size, dtype in zip(input_size, dtypes):
        ...
        if isinstance(size,list):
            ...
            if isinstance(size[0],tuple):
                tmp=[]
                for si in size[0]:
                    if isinstance(si,tuple): # if a high-dim tensor
                    ...
                    tmp.append(result)
                    # temporal solution
                    tmp=[torch.Tensor([1,0,1]).to(torch.long).to(device),
                         torch.Tensor([0,1]).to(torch.long).to(device),
                         torch.Tensor([1,0]).to(torch.long).to(device)]
                x.append(tmp)
                continue
            else:
                ...
                result = input_tensor.to(device).type(torch.long)
                result=torch.Tensor([[0,1],[0,1],[0,1]]).to(torch.long).to(device)
                x.append(result)
                continue
